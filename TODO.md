...
The duplicate-reachable-IP case shipped: `AliasIp` on `MonitoringInterface` with fwmark/DNAT policy routing, live-verified on real UCGF hardware against two devices sharing `192.168.100.1` on different WANs. The natively-monitored-WAN advisory has since been built (`NativeMonitoredWanDetector` matches UniFi's per-WAN ISP, falling back to the friendly WAN name). One follow-up idea remains open (not built):
