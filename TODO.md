...
- **Status:** built (`AliasIp` on `MonitoringInterface`, fwmark/DNAT policy routing). Verified live on real UCGF hardware (2026-07-05) against the exact motivating scenario: a Telekom ONT and a Starlink dish both at `192.168.100.1` on different WANs.
- **Gotcha found during live verification, Starlink specifically:**
  - **Symptom:** UniFi's own native Starlink dashboard widget (Obstructions/Dish Latency/Throughput) silently breaks if **any** Monitoring Interface (aliased or not) claims the main routing table's `/32` route to the address Starlink's dish answers on (`192.168.100.1` in this setup). This happened even for a *plain, non-aliased* Monitoring Interface on the *other* WAN (the ONT) - not just when both devices were aliased against each other.
  - **Root cause:** UniFi's own polling appears to depend on that address resolving toward the Starlink WAN via the ambient main table, not a dedicated mechanism.
  - **Rule of thumb:** the device sharing its IP with a UniFi-native-monitored WAN (Starlink today; possibly other UniFi-native integrations later) must always be the one that gets `AliasIp` set, never the "plain" one - so our feature never claims the shared IP's main-table slot at all. Confirmed live: with the ONT correctly aliased and no plain interface at all on either WAN, `ip route show <ip>/32` in the main table is empty and Starlink's native widget is unaffected.
- **Possible follow-up enhancement:** NetworkOptimizer already knows each WAN's underlying connection type (used elsewhere for WAN labeling). When a user picks a WAN identified as Starlink (or any other WAN type with known UniFi-native monitoring that depends on ambient reachability) as the **plain** (non-aliased) side of a Monitoring Interface, proactively warn and suggest aliasing it instead - rather than requiring a user to discover the breakage live, as happened here. Not yet implemented; flagging so the next person touching this area has the context.
- **Possible follow-up enhancement: LAN-client passthrough to the real IP.** An aliased device's own web UI can hard-redirect a browser to its real IP once loaded (confirmed live: the Telekom Glasfaser-Modem 2's UI redirects to `http://<real-ip>/interception.html`, presumably an anti-DNS-rebinding check) - so a human browsing the alias still hits a dead end unless something *also* routes their specific LAN IP to the real target. Today this requires a manual, non-persistent `ip rule` reusing the alias's own private table (`ip rule add from <client-ip> to <real-ip>/32 lookup <table>`) - works, but isn't managed by NetworkOptimizer. A clean built-in version is plausible: the private table already has the needed route, and the existing SNAT rule has no source restriction, so a passthrough client would get a working return path "for free" - likely just one additional per-row `ip rule` in the generated boot script, source-scoped to an admin-specified allowlist of LAN IPs. Deliberately **not building this now** - traded off as a rare personal-convenience need (occasional browser access) against real risk (a "grant direct real-IP access" knob sitting right next to a mechanism whose entire point is to never expose the real IP invites exactly the dual-WAN misconfiguration this feature exists to prevent) and against bundling scope into an already-large contribution. Gauge actual demand before building.
