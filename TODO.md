...
The duplicate-reachable-IP case shipped: `AliasIp` on `MonitoringInterface` with fwmark/DNAT policy routing, live-verified on real UCGF hardware against two devices sharing `192.168.100.1` on different WANs. Two follow-up ideas remain open (neither built):

- **Warn when a UniFi-native-monitored WAN is picked as the plain side.** UniFi's own native Starlink widget (Obstructions/Dish Latency/Throughput) silently breaks if **any** Monitoring Interface claims the main routing table's `/32` for the address the dish answers on - its polling depends on that address resolving toward the Starlink WAN via the ambient main table. So the device sharing its IP with a UniFi-native-monitored WAN must always be the aliased side, never the plain one. NetworkOptimizer already knows each WAN's connection type, so it could detect this at deploy time and warn (suggest aliasing that side) rather than letting the user hit the breakage live.
